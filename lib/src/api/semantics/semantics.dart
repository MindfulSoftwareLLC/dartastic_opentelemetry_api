...
///
/// This file is the home for every typed attribute-key enum in the
/// API. Each enum mirrors a top-level namespace from the OTel
/// [attribute registry](https://opentelemetry.io/docs/specs/semconv/attributes-registry/).
///
/// For the closed set of valid *values* for attributes like
/// `db.system.name` or `cloud.provider`, see `semantic_values.dart`.
library;

/// Base interface for OpenTelemetry semantic-convention enums. Every
/// attribute-key enum below implements this; consumers can also
/// implement it on their own app-specific enums to mix into
...
/// End-user identity attributes (the actor of a request).
/// [Specification](https://opentelemetry.io/docs/specs/semconv/attributes-registry/user/)
///
/// Note: `enduser.*` (see `ErrorResource`-style `Enduser` enum) is the
/// service-side view of an end user; `user.*` here is the actor itself.
enum User implements OTelSemantic {
  userId('user.id'),
  userEmail('user.email'),
  userFullName('user.full_name'),
  userName('user.name'),
  // Plural is the current OTel form; users typically have many roles.
  userRoles('user.roles'),
  // `user.session` ties a user to a session.id without forcing a join.
...
/// Session attributes — a logical user session spanning multiple events.
/// [Specification](https://opentelemetry.io/docs/specs/semconv/attributes-registry/session/)
